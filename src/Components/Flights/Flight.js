...
export default function Flight({flight}) {
    ...
    return (
        <tr style={{marginBottom: "14px"}}>
            ...
            <td>{user && user.admin &&
            <IconButton aria-label="edit" className="btn btn-primary" style={{color: '#440047'}}
                        onClick={updateFlight}><EditIcon/></IconButton>}</td>
            <td>{user && user.admin &&
            <IconButton aria-label="delete" className="btn btn-primary" style={{color: '#440047'}} onClick={() => {
                deleteFlight(flight.flight_number)
